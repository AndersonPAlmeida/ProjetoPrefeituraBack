...
class ShiftPolicy < ApplicationPolicy
  class Scope < Scope
    def resolve
      ...
      when "atendente_local"
        scope.where(professional_performer_id: professional.id)
  ...
  # Generic method for checking permissions when creating/modifying 
  # shifts. It is used for avoiding code repetition in shift's policy
  # methods.
  #
  # @param user [Array] current citizen and the permission provided
  # @return [Boolean] true if allowed, false otherwise
  def modify_policy(user)
    citizen = user[0]
    permission = Professional.get_permission(user[1])
    ...
    when permission == "adm_prefeitura"
      return (record.service_place.city_hall_id == city_hall_id)
  ...
  # Generic method for checking permissions when showing/accessing
  # shifts. It is used for avoiding code repetition in shift's policy
  # methods.
  #
  # @param user [Array] current citizen and the permission provided
  ...
  def access_policy(user)
    ...
    when "atendente_local"
      return (record.professional_performer_id == professional.id) 
